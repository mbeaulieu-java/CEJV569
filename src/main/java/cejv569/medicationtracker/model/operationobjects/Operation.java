...
/**
 * Operation is the base class for all the Operation sub classes in the program, those
 ...
 * Each Operation subclass must implement a ViewOperationInterface to process requests from
 ...
 * To pass on those data requests to the database layer, each Operation subclass thus has
 * access to the next layer via it's transaction property/attribute (which is a DataTransaction child class).
 * Each Operation child class inherits this property from Operation.
 *  As the middle layer, the Operation child classes perform any data validation required
